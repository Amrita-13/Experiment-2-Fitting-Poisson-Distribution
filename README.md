...
# Count frequency of each number in L
for i in range(M + 1):
    c = 0
    for j in range(N):
        if L[j] == i:
...
# Reset p and initialize other lists
p = list()
E = list()
xi = list()
...
# Calculate Poisson probabilities, expected frequencies, and chi-square terms
for x in range(M + 1):
